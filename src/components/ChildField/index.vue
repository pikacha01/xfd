...
const percentage = computed(() => {
  if(formStore.currentFormSteps?.data.initData[props.data.id] === null) return 0
...
const getTotal = async () =>{
  ...
  //@ts-ignore
  total.value =  res.data.initData["NumberField_74"]
}
...
<uni-forms-item :label="data.label" :name="data.id" :required="data.required">
  <view class="progressBox" @click="goToScan">
    ...
    <view class="num">{{ formStore.currentFormSteps!.data.initData[props.data.id] === null ? 0 : Number(formStore.currentFormSteps?.data.initData[props.data.id].length) }} / {{ total }}</view>
