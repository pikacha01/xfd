...
const calculate = computed(() => {
  ...
  return Number(formStore.currentFormSteps?.data.initData.NumberField_73) * Number(formStore.currentFormSteps?.data.initData.NumberField_74)
})
...
<uni-forms-item :label="data.label" :required="data.required" :name="data.id">
  <view class="inputContainer">
    ...
    disabled
    />
    <text v-if="data.unit">{{ data.unit }}</text>
  </view>
</uni-forms-item>
</template>
